...
class KnexAdapter {
   ...
   *
   * Undefined values should fallback to null
   * because some policies do not require all v[x] fields.
   *
   * @param {*} ptype The policy type.
   * @param {*} rule A list of rules for this policy.
   */
  createPolicy(ptype = 'p', rule = []) {
    return {
      ptype,
      v0: rule[0] || null,
      v1: rule[1] || null,
  ...
  async addPolicy(sec = '', pType = 'p', rule = []) {
    const policy = this.createPolicy(pType, rule);
    await this.policies.insert(policy);
  }

  async removePolicy(sec = '', pType = 'p', rule = []) {
    const policy = this.createPolicy(pType, rule);
    await this.policies.delete().where(policy);
  }

  async removeFilteredPolicy(sec = '', pType = 'p', rule = []) {
    const filteredPolicy = this.createFilteredPolicy(pType, rule);
